...
import { UMS_SERVICE, JobMessages } from "@core-api/microservices-utils";
...
export class JobsService {
  ...
  async findAll(filter: IAppCenterJobFilter): Promise<INonPagingResponse<IAppCenterJobResponse>> {
    try {
      const response = await this.umsClient.send({ cmd: JobMessages.GET_ALL_JOBS }, filter || {}).toPromise();
      return response
    } catch (error) {
  ...
  async findOne(id: number): Promise<IAppCenterJobResponse> {
    try {
      const response = await this.umsClient.send({ cmd: JobMessages.GET_ONE_JOB }, id).toPromise();
      return response
    } catch (error) {
      throw new NEST_COMMON.UnauthorizedException();
    }
  ...
  async update(jobId: number, updateJobDto: UpdateJobDto) {
    try {
      const response = await this.umsClient.send({ cmd: JobMessages.UPDATE_JOB }, {
        jobId,
        updateJobDto
      }).toPromise();
      return response
  ...
  async patchUpdate(jobId: number, updateJobDto: Partial<UpdateJobDto>) {
    try {
      const response = await this.umsClient.send({ cmd: JobMessages.PATCH_UPDATE_JOB }, {
        jobId,
        updateJobDto
      }).toPromise();
      return response
  ...
  async create(requestedJob: CreateJobDto): Promise<IAppCenterJobResponse> {
    try {
      const response = await this.umsClient.send({ cmd: JobMessages.CREATE_JOB }, requestedJob).toPromise();
      return response
    } catch (error) {
      throw new NEST_COMMON.UnauthorizedException();
    }
  ...
  async delete(id: number): Promise<void> {
    try {
      const response = await this.umsClient.send({ cmd: JobMessages.DELETE_JOB }, id).toPromise();
      return response
    } catch (error) {
      throw new NEST_COMMON.UnauthorizedException();
    }
